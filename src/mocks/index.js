import { calculatePoints, createMockTransaction, createMockUser, getMonthName } from "utils";

const users = createMockUser(3);
const transactions = createMockTransaction(users, 12);
const pointsCache = {};
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!result) {
      ...
      result = pointsCache[user.id].monthly ?? transactions.filter(transaction => transaction?.initiatedBy === user?.name).reduce((prev, curr) => {
        const month = getMonthName(new Date(curr.createdAt).getMonth()), points = calculatePoints(curr.amount);
...
export function fetchUserPoints(userId, delay = 300) {
  return new Promise((resolve, reject) => {
    ...
    if (!result) {
      ...
      const points = userTransactions.reduce((prev, curr) => prev + calculatePoints(curr?.amount), 0);
